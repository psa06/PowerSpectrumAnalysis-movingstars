...
def good_draw_LC(params, map, time, err_data):
    ...
    if x_start + (time[-1] - time[0]) * v_x <= len(map) and y_start + (time[-1] - time[0]) * v_y <= len(
        ...
        temp = np.multiply(-2.5, np.log10(map[
                                              path_y, path_x]))  # np.random.normal(0, np.mean(err_data), len(path_y)))  # -2.5 log() to convert flux into mag
...
def draw_LC_2maps(params_A, params_B, mapA, mapB, time, err_data):
    ...
    if lc_A is not None and lc_B is not None:
        lc = lc_A - lc_B
        plt.plot(time, lc_A, '.r')
        plt.plot(time, lc_B, '.b')
        plt.plot(time, lc, '.k')
        plt.show()
...
def power_spectrum(param, map, time, err_data, f, max_deriv):
    lc = good_draw_LC(param, map, time, err_data)
    if lc is not None and max(np.abs(np.diff(lc))) <= max_deriv:
        frequency, power = scipy.signal.periodogram(lc, f, window='flattop')
...
#print(n_cpu)
pool = multiprocessing.Pool(int(2))

print "+++++++++++++++"
x = pool.map(f, range(int(2)))
print x
print "+++++++++++++++"
...
for r0 in list_r0:
    print comb
    print r0
    map_A = datadir + mapdir + "convolved_map_A3_fft_thin_disk_%s_fml09.fits" % (r0)
    img_A = fits.open(map_A)[0]
    final_map_A = img_A.data[:, :]

    map_B = datadir + mapdir + "convolved_map_B2_fft_thin_disk_%s_fml09.fits" % (r0)
    ...
    for v_source in list_v:
        ...
        print len(var_power)
        print len(freq)
        with open(resultdir + 'powerspectrum/pkl/M0.3/spectrum_%s-%s_%s_%s_R%s_thin-disk_2maps_16.pkl' % (
