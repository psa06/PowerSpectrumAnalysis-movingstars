...
class f2nimage:
    ...
    def makepilimage(self, scale="log", negative=False):
        ...
        if scale == "clog" or scale == "clin":  # Rainbow !
            ...
            if scale == "clog":
                calcarray = 10.0 + 990.0 * (calcarray.clip(min=self.z1, max=self.z2) - self.z1) / (
    ...
    def drawcircle(self, x, y, r=10, colour=None, label=None):
        ...
        if label != None:
            ...
            self.draw.text((pilx - float(textwidth) / 2.0 + 2, pily + pilr + 4), label, fill=colour,
                           font=self.labelfont)
    ...
    def drawrectangle(self, xa, xb, ya, yb, colour=None, label=None):
        ...
        if label != None:
            ...
            self.loadlabelfont()
            textwidth = self.draw.textsize(label, font=self.labelfont)[0]
            self.draw.text(((pilxa + pilxb) / 2.0 - float(textwidth) / 2.0 + 1, pilya + 2), label, fill=colour,
                           font=self.labelfont)
    ...
    def tonet(self, outfile):
        ...
        if self.verbose:
            ...
            outfile, self.pilimage.size[0], self.pilimage.size[1], self.pilimage.mode)
...
def compose(f2nimages, outfile):
    ...
    verbose = np.any(
        np.array(verbosity))  # So we set the verbosity used in this function to true if any of the images is verbose.
    ...
    if verbose:
        ...
        outfile, compoimg.size[0], compoimg.size[1], compoimg.mode)
